...
class _HospitalListScreenState extends State<HospitalListScreen> {
  ...
  Future<void> _getNearbyHospitals() async {
    ...
    if (_currentPosition != null) {
      ...
      setState(() {
        _hospitals = hospitals.sublist(0, 5);
      });
    }
  }
  ...
  Future<double> _fetchTrafficScore(double latitude, double longitude) async {
    String apiKey = 'AIzaSyCjYbqJfSiVxFTWL3NQ8VA8ca9LgN5trQ4';
    String url = 'https://your_traffic_api_url?lat=$latitude&lng=$longitude&key=$apiKey';

    var response = await http.get(Uri.parse(url));
    var data = json.decode(response.body);

    // Parse the response and extract the traffic score
    double trafficScore = data['score'];

    return trafficScore;
  }

  Duration _calculateTravelTime(double distance, double trafficScore) {
    // Calculate travel time based on distance and traffic score
    // Modify this calculation based on your criteria
    double travelTimeInMinutes = distance / 50 * (1 + trafficScore);
    return Duration(minutes: travelTimeInMinutes.toInt());
  }
